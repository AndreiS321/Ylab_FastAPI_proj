...
async def create_menu(menu, session: AsyncSession) -> MenuOut:
    menu = Menu(title=menu['title'], description=menu['description'])
    session.add(menu)
    await session.flush()
    menu = await menu.to_pydantic_model(session)
    menu.id = int(menu.id)
...
async def create_submenu(submenu, menu_id: int, session: AsyncSession) -> SubmenuOut:
    submenu = Submenu(
        title=submenu['title'], description=submenu['description'], menu_id=menu_id
    )
    session.add(submenu)
    await session.flush()
    submenu = await submenu.to_pydantic_model(session)
